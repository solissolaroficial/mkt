...
import { RAW_BUDGET_ITEMS, OFFLINE_ACTIONS_DATA, REP_NAMES, MONTHS, FULL_MONTH_MAP } from '../constants';
import { OfflineAction } from '../types';

interface MarketingBudgetViewProps {
  onBack: () => void;
...
const MarketingBudgetView: React.FC<MarketingBudgetViewProps> = ({ onBack, selectedMonth }) => {
  // --- STATE ---
  const [requests, setRequests] = useState<OfflineAction[]>(OFFLINE_ACTIONS_DATA);
  ...
  const budgetItems = useMemo(() => {
      return RAW_BUDGET_ITEMS.filter(item => item.codObj === '13' && item.codGrp === '41');
  }, []);
