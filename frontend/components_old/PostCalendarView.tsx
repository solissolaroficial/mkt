...
import { CalendarPost, PostCategory, PostHistoryEvent, PostStatus, PostType } from '../types';
...
const PostCalendarView: React.FC<PostCalendarViewProps> = ({ posts, onUpdatePosts }) => {
  ...
  const handleStatusChange = (newStatus: PostStatus, comment?: string) => {
    ...
    // Se for publicar, abrir modal de confirmação
    ...
    const updatedPosts = posts.map(p => 
        p.id === selectedPost.id 
            ? { 
                ...p, 
                status: 'published', 
                publishedPlatforms: publishingPlatforms, // Salva as plataformas confirmadas
  ...
  // Usado apenas na criação do post
  ...
  const handleCreatePost = (e: React.FormEvent) => {
    ...
    const newPost: CalendarPost = {
        ...
        publishedPlatforms: [], // Inicia vazio
        history: [{
            id: Date.now().toString(),
            action: 'upload',
            user: 'Jackson',
                        ...
                        <div className="h-12 border-b border-gray-800 flex items-center justify-between px-4 text-white">
                            ...
                            <div className="flex gap-4">
                                ...
                                <div className="relative">
                                    ...
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>
                                </div>
                ...
                <div className="w-full md:w-1/2 flex flex-col h-full border-l border-gray-800">
                    ...
                    <div className="p-6 border-t border-gray-800 bg-[#0f1115]">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                             <button 
                                onClick={() => { setAdjustComment(''); handleStatusChange('adjust', 'Solicitado Ajuste'); }} // Just toggle UI state actually handled by rendering input above if needed, but for simplicity let's assume direct status set triggers input need or use alert
