...
import { PdvPost, RecurrentPdv } from '../types';
import { MOCK_PDV_POSTS, REP_NAMES, RECURRENT_PDVS } from '../constants';
...
const PdvTrackingView: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'posts' | 'missing'>('posts');
  const [posts, setPosts] = useState<PdvPost[]>(MOCK_PDV_POSTS);
  const [pdvList, setPdvList] = useState<RecurrentPdv[]>(RECURRENT_PDVS);
  ...
  const [newPostPlatform, setNewPostPlatform] = useState<'instagram' | 'facebook' | 'linkedin' | 'tiktok'>('instagram');
