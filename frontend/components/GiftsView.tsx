...
import { GiftItem, GiftTransaction, Notification } from '../types';
import { GIFT_ITEMS, GIFT_TRANSACTIONS_MOCK, REP_NAMES } from '../constants';
...
const GiftsView: React.FC<GiftsViewProps> = ({ onAddNotification }) => {
  // State for items (initialized from constants)
  ...
  const [transactions, setTransactions] = useState<GiftTransaction[]>(() => {
    const existingItems = new Set(GIFT_TRANSACTIONS_MOCK.map(t => t.itemName));
    const initialData: GiftTransaction[] = [];
    
    GIFT_ITEMS.forEach((item, idx) => {
    ...
    return [...GIFT_TRANSACTIONS_MOCK, ...initialData];
  });
