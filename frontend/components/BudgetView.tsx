...
import { RAW_BUDGET_ITEMS, MONTHS } from '../constants';
...
const BudgetView: React.FC<BudgetViewProps> = ({ onBack, filterFn, customTitle, customSubtitle }) => {
  const [dataItems, setDataItems] = useState<BudgetItem[]>(() => {
      return filterFn ? RAW_BUDGET_ITEMS.filter(filterFn) : RAW_BUDGET_ITEMS;
  });
  const [searchTerm, setSearchTerm] = useState('');
  
  // Edit State: Now includes type (budget or realized)
  const [editingCell, setEditingCell] = useState<{ id: string, colIndex: number, type: 'budget' | 'realized' } | null>(null);
  ...
  const hierarchy = useMemo(() => {
      ...
      if (filterFn) {
          ...
          // We can't easily update state during render, but we can rely on initial state or effects if needed.
          // For now, let's just let the user expand or rely on the default state which covers major groups.
      }
  ...
  }, [searchTerm, hierarchy.length]); // Dependency on filtered result length roughly
