...
import { Store, Megaphone, DollarSign, MapPin, Calendar, CheckCircle, XCircle, Clock, Target, Globe, LayoutGrid, List, ArrowLeft, ChevronRight, User, Package, Handshake, Award, MonitorPlay, Tent, MessageSquareText, X, Filter, Plus, Save, Trash2, Edit, History } from 'lucide-react';
import { MOCK_SHOWROOM_ITEMS, REPS_MARKETING_DATA, FULL_MONTH_MAP, OFFLINE_ACTIONS_DATA, MONTHS, REP_NAMES } from '../constants';
import RepTable from './RepTable';
import PdvTrackingView from './PdvTrackingView';
import { OfflineAction, ShowroomItem } from '../types';

interface CooperativeActionsViewProps {
...
const CooperativeActionsView: React.FC<CooperativeActionsViewProps> = ({ selectedMonth = '---', onRepClick }) => {
  const [activeSubTab, setActiveSubTab] = useState<'menu' | 'marketing' | 'online_marketing' | 'showroom' | 'offline_marketing'>('menu');
  
  // Offline Actions State
  const [offlineActions, setOfflineActions] = useState<OfflineAction[]>(OFFLINE_ACTIONS_DATA);
  const [offlineFilterCategory, setOfflineFilterCategory] = useState('TODAS');
  ...
  const getOfflineCategory = (action: OfflineAction) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in zoom-in-95 duration-300">
              {menuOptions.map((option) => {
                  ...
                  return (
                      <button
                          key={option.id}
                          onClick={() => setActiveSubTab(option.id as any)}
                          className={`
                    ...
                    <div className="flex flex-wrap gap-4">
                        ...
                        <div className="relative">
                            <select
                                ...
                                onChange={(e) => setOfflineFilterCategory(e.target.value)}
                ...
                <form onSubmit={handleAddOfflineAction} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
                    ...
                    <div>
                        ...
                        <select 
                            ...
                            onChange={(e) => setNewActionForm({...newActionForm, category: e.target.value})}
                            className="w-full bg-[#0f1115] border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 focus:outline-none focus:ring-1 focus:ring-[#1e5144]"
                        >
                            {OFFLINE_CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                        </select>
